...
public class StartupHelper {
	...
	public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
		...
		if (!ManagementFactory.getRuntimeMXBean().getInputArguments().contains(ENABLE_NATIVE_ACCESS_ARG)) {
			jvmArgs.add(ENABLE_NATIVE_ACCESS_ARG);
		}
	...
	private static long getProcessID(boolean isMac) {
		...
		if (separator > 0) {
			try {
				return Long.parseLong(runtimeName.substring(0, separator));
			} catch (NumberFormatException ignored) {
				// Fall through to the sentinel below if the runtime name is unexpected.
			}
		}
		return -1L;
	}
